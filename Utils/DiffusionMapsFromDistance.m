...
%     K_sorted_val=sort(K(:));
%     th_low_prec=0.01;th_low_val=K_sorted_val(floor(th_low_prec*length(K(:))));
%     th_high_prec=0.99;th_high_val=K_sorted_val(ceil(th_high_prec*length(K(:))));
%     K((K<=th_low_val))=th_low_val;
%     K((K>=th_high_val))=th_high_val;
%     figure(); imagesc(K);
    % figure(); hist(K(:),1e3)
